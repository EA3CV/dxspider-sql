...
use vars qw($progname $version $subversion $build $gitversion $gitbranch);
require Version;
...
sub cease
{
	...
	LogDbg('cluster', "$progname v$version build $build (git: $gitbranch/$gitversion) using perl $^V on $^O ended");
...
sub setup_start
{
	...
	# try to load Encode and Git
	{
		...
		# determine the real Git version/build/commit (robust)
		my ($tag, $count, $commit, $branch);

		eval { $tag = `git -C $root describe --tags --abbrev=0 2>/dev/null`; chomp $tag if $tag; };
		eval { $count = `git -C $root rev-list --count HEAD 2>/dev/null`; chomp $count if defined $count; };
		eval { $commit = `git -C $root rev-parse --short HEAD 2>/dev/null`; chomp $commit if $commit; };
		eval { $branch = `git -C $root rev-parse --abbrev-ref HEAD 2>/dev/null`; chomp $branch if $branch; };

		$version = $tag if $tag;
		$build   = $count if defined $count && $count =~ /^\d+$/;

		$gitversion = $commit if $commit;
		$gitbranch  = ($branch && $branch ne 'HEAD') ? $branch : $gitbranch;
		$SIG{__DIE__} = $w;
	}
	...
	my ($year) = (gmtime)[5];
	$year += 1900;
	LogDbg('cluster', "$progname v$version build $build (git: $gitbranch/$gitversion) using perl $^V on $^O started");
