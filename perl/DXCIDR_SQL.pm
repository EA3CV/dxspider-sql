...
sub _ensure_table {
	...
	if (!$exists) {
		dbg("[DXCIDR_SQL] Creating table: $table");
	}

	if ($main::db_backend eq 'sqlite') {
		$dbh->do("CREATE INDEX IF NOT EXISTS ${table}_list_type_idx ON $table (list_type)");
		$dbh->do("CREATE INDEX IF NOT EXISTS ${table}_ts_idx ON $table (ts)");
	}
	...
	if ($main::db_backend eq 'mysql') {
		...
		while (my $row = $sth->fetchrow_hashref) {
			$cols{$row->{Field}} = 1 if $row->{Field};
		}
	} else {
		my $sth = $dbh->prepare("PRAGMA table_info($table)");
		$sth->execute();
		while (my $row = $sth->fetchrow_hashref) {
			$cols{$row->{name}} = 1 if $row->{name};
		}
	}

	for my $c (qw(ip ts list_type)) {
		die "[DXCIDR_SQL] table '$table' exists but is missing required column '$c'"
			unless $cols{$c};
	}
}
