...
sub load
{
	...
	# tie the main prefix database (BTREE) - in memory by default
	...
	if (-e $fn) {
		...
		# If we're using an SQL backend, we can rebuild from the DB as a fallback
		if ($main::db_backend && $main::db_backend ne 'file') {
			require Prefix_SQL;
			my $sql = Prefix_SQL->new();
			$sql->load_into(\%pre, \%prefix_loc);
			dbg("Prefix: Loaded from SQL backend: $main::db_backend");
		} else {
			die "Prefix.pm: cannot find $fn, have you run /spider/perl/create_prefix.pl?";
		}
	}

	# If SQL backend is selected, persist what we just loaded into SQL
	if (!$out && $main::db_backend && $main::db_backend ne 'file') {
		require Prefix_SQL;
		my $sql = Prefix_SQL->new();
		eval { $sql->store_from(\%pre, \%prefix_loc); };
		$out .= $@ if $@;
	}
